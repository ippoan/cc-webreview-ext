...
- `<!-- web-review -->` マーカー付きコメントが**この PR での自分の最後のコメント**である
  場合のみ、新規投稿せず
  `gh pr comment {{PR_NUMBER}} --repo {{PR_REPO}} --edit-last --body "..."` で
  そのコメントを更新する (再走・リトライで同内容を重複投稿しない)。
  **注意: `--edit-last` はマーカーに関係なく「自分の最後のコメント」を編集する** —
  マーカー付きコメントの後に自分の別コメントがある場合に使うと取り違える
- マーカー付きコメントが他人の投稿の場合、またはマーカー付きコメントが自分の投稿でも
  **最後のコメントではない**場合は、投稿を skip し、その旨を報告する
- マーカー付きコメントが無ければ
  `gh pr comment {{PR_NUMBER}} --repo {{PR_REPO}} --body "..."` で新規投稿する
