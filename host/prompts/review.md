...
5. (ブラウザツールが使える場合のみ) `gh pr view {{PR_NUMBER}} --repo {{PR_REPO}} --comments` の
   既存コメントに `<!-- pr-chat-bridge:request -->` で始まるブラウザ検証依頼があり、それより
   **後**に `<!-- pr-chat-bridge:result -->` の回答コメントがまだ無い場合は、依頼本文の
   ...
   - ブラウザツールが使えない場合はこの手順を丸ごと skip し、Web Review コメントに
     「ブラウザ検証依頼は未実施 (browser ツール無し)」と 1 行残す
6. 指摘は 正しさ → 設計 → 可読性 の順で重大度を付けて整理する
